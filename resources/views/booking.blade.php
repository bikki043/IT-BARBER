...
<head>
    ...
    <style>
        ...
        .date-scroller {
            ...
            padding: 20px 5px; scroll-behavior: smooth;
        }
        .date-card {
            min-width: 100px; height: 130px;
        ...
        .date-radio:checked+.date-card {
            ...
            transform: translateY(-10px); box-shadow: 0 15px 30px rgba(255, 204, 0, 0.3);
        }
        ...
        .lux-card img {
            ...
            filter: brightness(0.6); transition: var(--transition);
        }
        .lux-card-body {
            position: absolute; bottom: 0; left: 0; right: 0;
            padding: 20px; background: linear-gradient(to top, rgba(0, 0, 0, 0.9) 40%, transparent);
        }
        
        .service-radio:checked+.lux-card { 
            border: 3px solid var(--gold); 
        ...
        .input-lux {
            ...
            color: #fff !important; height: 55px !important;
            border-radius: 15px !important; margin-bottom: 20px;
        }
        ...
        .btn-lux-submit {
            ...
            padding: 20px; border-radius: 20px; font-family: 'Oswald';
            font-weight: 700; font-size: 1.5rem; text-transform: uppercase;
            letter-spacing: 2px; transition: var(--transition);
            cursor: pointer;
        }
        .btn-lux-submit:hover:not(:disabled) { transform: translateY(-5px); box-shadow: 0 20px 40px var(--gold-glow); background: #fff; }
        .btn-lux-submit:disabled { opacity: 0.6; cursor: not-allowed; }
    </style>
</head>
...
<body>
    <div class="container">
        ...
        @if ($errors->any())
            <div class="alert alert-danger mb-4 shadow-lg" style="border-radius: 20px; background: rgba(220,53,69,0.95); color: #fff; border: 2px solid #fff;">
                <h5 class="alert-heading font-weight-bold"><i class="fas fa-exclamation-triangle mr-2"></i> พบข้อผิดพลาด:</h5>
                <ul class="mb-0">
        ...
        <div class="brand-box">
            ...
            <h1 class="shop-title"> IT BARBER</h1>
                    ...
                    <div class="scroller-wrapper">
                        <div class="date-scroller">
                            ...
                            @for ($i = 0; $i < 14; $i++)
                                ...
                                @endphp
                                <label class="m-0">
                                    <input type="radio" name="appointment_date" value="{{ $val }}" class="date-radio d-none" {{ old('appointment_date') == $val || $i == 0 ? 'checked' : '' }}>
                                    <div class="date-card shadow">
                                        <span class="day text-muted small">{{ $d_name }}</span>
                                        <span class="num" style="font-size: 1.5rem; font-weight: 700;">{{ $d_num }}</span>
                        ...
                        @foreach ($services as $service)
                            <div class="col mb-4">
                                <label class="m-0 w-100 h-100">
                                    <input type="radio" name="service_id" value="{{ $service->id }}" class="service-radio d-none" {{ old('service_id') == $service->id ? 'checked' : '' }}>
                                    <div class="lux-card shadow-lg">
                                        @php
                                            $imagePath = $service->image_url;
                                            $displayImage = $imagePath && filter_var($imagePath, FILTER_VALIDATE_URL) ? $imagePath : ($imagePath ? asset('storage/' . $imagePath) : 'https://images.unsplash.com/photo-1599351431247-f10b21ce5602?q=80&w=600');
                                        @endphp
                                        ...
                                        <div class="lux-card-body">
                                            ...
                                            <div class="d-flex justify-content-between align-items-center">
                                                ...
                                                <span class="badge badge-light">{{ $service->duration }} MINS</span>
                                            </div>
                                        </div>
                                    </div>
                                </label>
                    ...
                    <div class="summary-panel shadow-2xl">
                        ...
                        <div class="form-group">
                            ...
                            <select name="branch" class="form-control input-lux" required>
                                <option value="บ้านโป่ง" {{ old('branch') == 'บ้านโป่ง' ? 'selected' : '' }}>BRANCH: BAN PONG</option>
                                <option value="หนองอ้อ" {{ old('branch') == 'หนองอ้อ' ? 'selected' : '' }}>BRANCH: NONG OR</option>
                            </select>
                        </div>
                        ...
                        <div class="form-group">
                            ...
                            <div class="time-select-grid">
                                @foreach (['10:00', '11:00', '12:00', '13:00', '14:00', '15:00', '16:00', '17:00', '18:00', '19:00'] as $t)
                                    <label class="m-0">
                                        <input type="radio" name="appointment_time" value="{{ $t }}" class="time-radio d-none" {{ old('appointment_time') == $t ? 'checked' : '' }}>
                        ...
                        <input type="hidden" name="barber_id" value="{{ $barbers->first()->id ?? 1 }}">

                        <button type="submit" id="submitBtn" class="btn-lux-submit shadow">CONFIRM NOW</button>
        ...
        <footer class="footer-info pb-5 text-center mt-5">
            <p class="mb-1 text-white-50">© 2026 IT BARBER STUDIO CO., LTD. ALL RIGHTS RESERVED.</p>
    ...
    $(document).ready(function() {
        ...
        $('#bookingForm').on('submit', function(e) {
            ...
            // ตรวจสอบค่าที่จำเป็น
            const date = $('input[name="appointment_date"]:checked').val();
            const service = $('input[name="service_id"]:checked').val();
            const time = $('input[name="appointment_time"]:checked').val();
            const name = $('input[name="customer_name"]').val();
            const phone = $('input[name="customer_phone"]').val();

            if(!date || !service || !time || !name || !phone) {
                alert('กรุณาเลือกวัน เวลา และกรอกข้อมูลให้ครบถ้วนครับ');
                e.preventDefault(); 
                return false;
            }
            ...
            // ปล่อยให้ฟอร์มส่งแบบปกติไปยัง Controller
            return true; 
        });
    }); // ปิดเครื่องหมายปีกกาที่หายไป
    </script>
</body>
</html>